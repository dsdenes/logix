...
function expression() {
  ...
  var variables = Object.assign({}, config.variables);
  var tree = traverse(config.tree);
  return {
    evaluate,
    mutate,
    getPath,
  ...
  function mutate() {
    ...
    try {
      ...
      if (_.inRange(rand, 0, 0.20)) {
        addRandomExpression(); // 20%
      } else if (_.inRange(rand, 0.2, 0.4)) {
        removeRandomExpression(); // 60%
      } else {
  ...
  function removeRandomExpression() {
    ...
    if (expressions.length > 1) {
      var _expression = _.sample(expressions);

      removeExpressionAtGroupPath(expressionGroupPath, _expression);
    }
  }
